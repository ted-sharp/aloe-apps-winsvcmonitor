...
using Aloe.Apps.ServiceMonitorClient.Services;
using System.IO;

namespace Aloe.Apps.ServiceMonitorClient;
...
public partial class MainWindow : Window
{
    ...
    private readonly Dictionary<string, ServiceMonitorLib.Models.ServiceInfo> _serviceStatuses = new();
    ...
    public MainWindow()
    {
        ...
        _options = configuration.GetSection("ServiceMonitor").Get<ServiceMonitorClientOptions>()
            ?? new ServiceMonitorClientOptions();
    ...
    private async void PollCriticalServices(object? state)
    {
        try
        {
            using var httpClient = new ServiceMonitorHttpClient(_options);
    ...
    private void StatusMonitor_ServiceStatusUpdated(object? sender, ServiceMonitorLib.Models.ServiceInfo service)
    {
        lock (_serviceStatuses)
        {
            _serviceStatuses[service.ServiceName] = service;
    ...
    private void UpdateCriticalServicesStatus()
    {
        ...
        lock (_serviceStatuses)
        {
            hasCriticalDown = _serviceStatuses.Values.Any(s =>
                s.IsCritical && s.Status != ServiceMonitorLib.Models.ServiceStatus.Running);
        }
